...
router.post('/accountNumberPrompt', (req, res) => {
  ...
  router.post('/accountNumber', (req, res) => {
    ...
    if (!digits && acctMenuErrCount < 3) {
        ...
        res.status(200).json(
            freeclimb.percl.build(
                freeclimb.percl.say('Error, please enter your six digit account number or press 0 to speak with an operator'),
        ...
        res.status(200).json(
            freeclimb.percl.build(
                freeclimb.percl.redirect(`${host}/confirmAccountNumberPrompt?acct=${digits}`)
            )
        )
    }
  })
