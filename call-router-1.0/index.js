...
app.post('/incomingCall', (req, res) => {
    res.status(200).json(freeclimb.percl.build(
...
app.post('/mainMenuPrompt', (req, res) => {
    res.status(200).json(freeclimb.percl.build(
        freeclimb.percl.getDigits(`${host}/mainMenu`, {
            prompts: [freeclimb.percl.say('Press 1 for existing orders, 2 for new orders, or 0 to speak to an operator')],
            maxDigits: 1,
            minDigits: 1,
            flushBuffer: true
        })
...
app.post('/mainMenu', (req, res) => {
    ...
    const menuOpts = new Map([
        ['1',{script: "Redirecting your call to existing orders.",redirect: `${host}/endCall`}],
        ['2', {script: "Redirecting your call to new orders.",redirect: `${host}/endCall`}],
...
app.post('/endCall', (req, res) => {
    res.status(200).json(freeclimb.percl.build(
        freeclimb.percl.say("Thank you for calling the Node IVR sample app baseline, have a nice day!"),
